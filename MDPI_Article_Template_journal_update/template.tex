...
Run level (aggregated) & 0.833 & 1.000 & 0.909 & 0.969 \\
\bottomrule
\end{tabularx}
\end{table}
...
An independent external validation dataset collected from a separate fabrication batch or test campaign was not available for the current study. The present evidence should therefore be interpreted as a grouped within-campaign validation of a compact physics-guided detector. Within that scope, the combination of run-bootstrap uncertainty reporting and dual window-level/run-level reporting provides a transparent basis for judging performance.
